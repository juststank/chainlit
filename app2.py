        ...
        messages = [
            {
                ...
                "content": (
                    ...
                    "Device configuration in FortiManager has multiple layers:\n"
                    "1. Device metadata: get_current_device_config → device info, NOT full config\n"
                    "2. Device database config: The actual FortiGate configuration stored in FMG\n"
                    "3. Running config: What's actually on the device (need device query)\n\n"
                    
                    "**For router/interface/system config in device database:**\n"
                    "- Routes are stored in the DEVICE DATABASE, not in device metadata\n"
                    "- Need to access /pm/config/adom/{adom}/obj/fmg/device/{device}/router/static\n"
                    "- Or use CLI template provisioning to see what's configured\n"
                    "- get_current_device_config only shows device properties, not full config\n\n"
                    
                    "To get actual routes:\n"
                    "1. list_devices(adom='X') to get device names\n"
                    "2. Look for tools that access device database router config\n"
                    "3. May need to use system_proxy or direct device query\n"
                    "4. Routes might be in CLI templates applied to devices\n\n"
