        ...
        messages = [
            {
                ...
                "content": (
                    ...
                    "To get device-level configuration (routes, interfaces, etc.):\n"
                    "1. First get device name: list_devices(adom='X')\n"
                    "2. Try these tools for routes:\n"
                    "   - get_device_routing_configuration(device_name='Y', adom='X')\n"
                    "   - get_current_device_config(device_name='Y', adom='X') - full config\n"
                    "   - Device router settings may be in device database\n"
                    "3. If 'object does not exist' error:\n"
                    "   - Routes might be in device profiles or templates\n"
                    "   - Try list_device_profiles(adom='X')\n"
                    "   - Check if routes are in CLI templates\n"
                    "   - Routes may need to be fetched from actual device via proxy\n\n"
